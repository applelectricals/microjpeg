...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    compressionJobs = pgTable("compression_jobs", {
      id: serial("id").primaryKey(),
      userId: integer("user_id"),
      status: varchar("status", { length: 50 }),
      createdAt: timestamp("created_at").defaultNow()
    });
    leadMagnetSignups = pgTable("lead_magnet_signups", {
      id: serial("id").primaryKey(),
      email: varchar("email", { length: 255 }),
    ...
    fileProcessing = pgTable("file_processing", {
      id: serial("id").primaryKey(),
      userId: integer("user_id"),
    ...
    pageUsage = pgTable("page_usage", {
      id: serial("id").primaryKey(),
      sessionId: varchar("session_id", { length: 255 }),
      pageIdentifier: varchar("page_identifier", { length: 50 }),
      createdAt: timestamp("created_at").defaultNow()
    });
    rewardTransactions = pgTable("reward_transactions", {
      id: serial("id").primaryKey(),
      userId: integer("user_id"),
    ...
    userReferrals = pgTable("user_referrals", {
      id: serial("id").primaryKey(),
    ...
    adminAuditLogs = pgTable("admin_AuditLogs", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
    ...
    operationLog2 = pgTable("operation_Log", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
    ...
    apiCompressRequestSchema = pgTable("api_CompressRequestSchema", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
    ...
    createApiKeySchema = pgTable("create_ApiKeySchema", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
    ...
    anonymousSessionScopes = pgTable("anonymous_SessionScopes", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
    ...
    loginSchema = pgTable("login_Schema", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
    ...
    signupSchema = pgTable("signup_Schema", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
    ...
    specialFormatTrials = pgTable("special_FormatTrials", {
      id: serial("id").primaryKey(),
      referrerId: integer("referrer_id"),
      referredId: integer("referred_id"),
      code: varchar("code", { length: 100 }),
...
// server/storage.ts
import { eq, desc as desc2, inArray, and, gte } from "drizzle-orm";
  ...
  "server/storage.ts"() {
    ...
    DatabaseStorage = class {
      ...
      async getJobsByIds(ids) {
        ...
        const jobs = await db.select().from(compressionJobs).where(inArray(compressionJobs.id, ids));
        return jobs;
      }
      async updateCompressionJob(id, updates) {
        const [updatedJob] = await db.update(compressionJobs).set(updates).where(eq(compressionJobs.id, id)).returning();
        return updatedJob;
      }
      async getAllCompressionJobs(userId2) {
        const jobs = await db.select().from(compressionJobs).where(userId2 ? eq(compressionJobs.userId, userId2) : void 0).orderBy(desc2(compressionJobs.createdAt));
        return jobs;
      }
      async deleteCompressionJob(id) {
        const result = await db.delete(compressionJobs).where(eq(compressionJobs.id, id));
        return result.rowCount ? result.rowCount > 0 : false;
      }
      async getCompressionJobsBySession(sessionId) {
        const jobs = await db.select().from(compressionJobs).where(eq(compressionJobs.sessionId, sessionId)).orderBy(desc2(compressionJobs.createdAt));
        return jobs;
      }
      ...
      async togglePremiumStatus(userId2) {
        ...
        const [user] = await db.update(users).set({
          ...
          updatedAt: /* @__PURE__ */ new Date()
        }).where(eq(users.id, userId2)).returning();
        return user;
      }
      async updateUserStripeInfo(userId2, stripeCustomerId, stripeSubscriptionId) {
        ...
        const [user] = await db.update(users).set(updates).where(eq(users.id, userId2)).returning();
        return user;
      }
      ...
      async getUserShares(userId2, limit = 10) {
        const shares = await db.select().from(socialShares).where(eq(socialShares.userId, userId2)).orderBy(socialShares.sharedAt).limit(limit);
        return shares;
      }
      async getUserRewards(userId2) {
        const [rewards] = await db.select().from(userRewards).where(eq(userRewards.userId, userId2));
        return rewards;
      }
      async addRewardPoints(userId2, points, source, relatedId) {
        let [rewards] = await db.select().from(userRewards).where(eq(userRewards.userId, userId2));
        if (!rewards) {
          ...
          [rewards] = await db.update(userRewards).set({
            ...
            updatedAt: /* @__PURE__ */ new Date()
          }).where(eq(userRewards.userId, userId2)).returning();
        }
      ...
      async getUserReferral(userId2) {
        const [referral] = await db.select().from(userReferrals).where(eq(userReferrals.userId, userId2));
        return referral;
      }
      ...
      async getLeadMagnetSignup(email) {
        const [signup] = await db.select().from(leadMagnetSignups).where(eq(leadMagnetSignups.email, email));
        return signup;
      }
      ...
      async deleteLeadMagnetSignup(id) {
        const result = await db.delete(leadMagnetSignups).where(eq(leadMagnetSignups.id, id));
...
var R2Service, r2Config, requiredEnvVars, missingVars, r2Service, R2_FOLDERS, r2Service_default;
    ...
    R2Service = class {
      ...
      async uploadFile(filePath, fileName, options = {}) {
        try {
          ...
          const fileHash = crypto3.createHash("md5").update(fileName + Date.now()).digest("hex").substring(0, 8);
          const key = `${folder}/${date}/${fileHash}_${fileName}`;
          const fileContent = await fs3.promises.readFile(filePath);
  ...
  try {
    ...
    if (existingSettings) {
      await db.update(appSettings).set({
        ...
        updatedAt: /* @__PURE__ */ new Date(),
        updatedBy
      }).where(eq2(appSettings.id, existingSettings.id));
    } else {
...
async function hasSuperuserBypass(req) {
  try {
    ...
    const [user] = await db.select().from(users).where(eq2(users.id, session3.userId));
...
var SUPERUSER_EMAIL, SUPERUSER_PASSWORD, ADMIN_UI_ENABLED, ADMIN_SEED_TOKEN, ensureSuperuser, cachedAppSettings, settingsCacheTime, CACHE_DURATION;
...
import { eq as eq3, and as and2, sql } from "drizzle-orm";
var DualUsageTracker;
  ...
  "server/services/DualUsageTracker.ts"() {
    ...
    DualUsageTracker = class {
      ...
      async getCurrentUsage() {
        ...
        const usageResult = await db.select().from(userUsage).where(and2(
          eq3(userUsage.userId, this.userId || "anonymous"),
          eq3(userUsage.sessionId, this.sessionId)
        )).limit(1);
        ...
        if (needsUpdate) {
          await db.update(userUsage).set(updateData).where(and2(
            eq3(userUsage.userId, this.userId || "anonymous"),
            eq3(userUsage.sessionId, this.sessionId)
          ));
        }
...
var redisUrl, isUpstash, redisConfig, hasRedisConfig, connectionType, redis, SKIP_REDIS, safeRedis, redis_default;
...
var queueConfig, imageQueue, rawQueue, bulkQueue, QUEUE_PRIORITIES, JOB_TYPES;
var init_queueConfig = __esm({
  "server/queueConfig.ts"() {
    ...
    imageQueue = new Bull("image-processing", {
      redis: {
    ...
    rawQueue = new Bull("raw-processing", {
      redis: {
        host: redis.options.host,
        port: redis.options.port,
        password: redis.options.password,
    ...
    bulkQueue = new Bull("bulk-processing", {
      redis: {
        host: redis.options.host,
        port: redis.options.port,
        password: redis.options.password,
...
async function performCompression(inputPath, outputPath, options = {}) {
  ...
  const originalStats = await fs5.promises.stat(inputPath);
  const originalSize = originalStats.size;
  let sharpInstance = sharp4(inputPath);
  if (width || height) {
  ...
  const compressedStats = await fs5.promises.stat(outputPath);
...
async function processImageCompression(job) {
  ...
  try {
    ...
    try {
      ...
      try {
        await fs5.promises.unlink(outputPath);
...
async function processRAWConversion(job) {
  ...
  try {
    ...
    await fs5.promises.copyFile(filePath, outputPath);
    await job.progress(100);
...
async function processBulkCompression(job) {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const outputPath = path5.join("compressed", `${Date.now()}_${file.fileName}`);
...
var CompressionEngine = class _CompressionEngine {
  ...
  static async processRawWithDcraw(inputPath, outputPath, outputFormat, options = {}) {
    ...
    try {
      ...
      const rawBuffer = await fs2.readFile(inputPath);
      let dcrawOptions = {
        exportAsTiff: true,
  ...
  static async canDcrawProcess(filePath) {
    try {
      const rawBuffer = await fs2.readFile(filePath);
  ...
  static async compressToTargetSize(inputPath, outputPath, targetSize, options = {}) {
    ...
    while (iterations < maxIterations) {
      ...
      const stats2 = await fs2.stat(tempPath);
      ...
      if (Math.abs(currentSize - targetSize) / targetSize < 0.1) {
        await fs2.rename(tempPath, outputPath);
    ...
    const stats = await fs2.stat(`${outputPath}.temp`);
    await fs2.rename(`${outputPath}.temp`, outputPath);
    return {
  ...
  static async compressWithAdvancedSettings(inputPath, outputPath, quality, outputFormat, options = {}, timeoutMs = 12e4, originalFilename) {
    ...
    if (isRawFormat) {
      ...
      try {
        ...
        const tempStats = await fs2.stat(tempUncompressedPath);
        await fs2.unlink(tempUncompressedPath);
  ...
  static async generateResponsiveSizes(inputPath, outputDir, baseName, options) {
    ...
    for (const width of options.sizes) {
      const outputPath = path2.join(outputDir, `${baseName}-${width}w.${options.format === "jpeg" ? "jpg" : options.format}`);
      let sharpInstance = sharp(inputPath);
...
// server/routes.ts
init_db();
import { and as and6, eq as eq9, gt, sql as sql2 } from "drizzle-orm";
...
async function calculatePSNR(originalPath, compressedPath) {
  try {
    const [originalBuffer, compressedBuffer] = await Promise.all([
      sharp2(originalPath).removeAlpha().raw().toBuffer({ resolveWithObject: true }),
      sharp2(compressedPath).removeAlpha().raw().toBuffer({ resolveWithObject: true })
    ]);
...
async function calculateSSIM(originalPath, compressedPath) {
  try {
    const [originalBuffer, compressedBuffer] = await Promise.all([
      sharp2(originalPath).greyscale().removeAlpha().raw().toBuffer({ resolveWithObject: true }),
      sharp2(compressedPath).greyscale().removeAlpha().raw().toBuffer({ resolveWithObject: true })
    ]);
...
async function compressToTargetSize(jobId, file, targetSize, options) {
  try {
    ...
    const outputPath = path3.join("compressed", `${jobId}.jpg`);
...
var RazorpayService = class {
  ...
  verifyPaymentSignature(orderId, paymentId, signature) {
    ...
    try {
      ...
      const expectedSignature = crypto2.createHmac("sha256", process.env.RAZORPAY_KEY_SECRET).update(body).digest("hex");
...
var planGatingMiddleware = async (req, res, next) => {
  ...
  const pageConfig = PAGE_CONFIGS[path9];
  if (!pageConfig) {
    return next();
  }
...
var registerConversionRoutes = (app2) => {
  Object.keys(PAGE_CONFIGS).forEach((path9) => {
    console.log(`[ConversionRoutes] Registering universal format page: ${path9}`);
    app2.get(
      path9,
  ...
  Object.keys(CONVERSION_MATRIX).forEach((route) => {
    const config = CONVERSION_MATRIX[route];
    const path9 = `/${config.type}/${route}`;
    console.log(`[ConversionRoutes] Registering single conversion page: ${path9}`);
    app2.get(
      path9,
      conversionPageMiddleware,
      (req, res, next) => {
...
var subscriptionTierAccessControl = async (req, res, next) => {
  try {
    ...
    const [user] = await db.select({ subscriptionTier: users.subscriptionTier, subscriptionStatus: users.subscriptionStatus }).from(users).where(eq4(users.id, session3.userId)).limit(1);
    if (!user) {
      return next();
    }
    ...
    if (subscriptionTier === "test_premium" && subscriptionStatus === "active") {
      ...
      if (restrictions.blockedRoutes.some((route) => path9.startsWith(route))) {
        return res.status(403).json({
          error: "Access denied",
      ...
      if (path9 === "/free" || path9 === "/compress-free") {
...
var tierBasedRouting = async (req, res, next) => {
  try {
    const session3 = req.session;
    const path9 = req.path;
    if (!session3?.userId || !["/", "/dashboard", "/home"].includes(path9)) {
      return next();
    }
    const [user] = await db.select({ subscriptionTier: users.subscriptionTier, subscriptionStatus: users.subscriptionStatus }).from(users).where(eq4(users.id, session3.userId)).limit(1);
    if (!user) {
...
var apiTierAccessControl = async (req, res, next) => {
  try {
    const session3 = req.session;
    const path9 = req.path;
    if (!path9.startsWith("/api/") || !session3?.userId) {
      return next();
    }
    const [user] = await db.select({ subscriptionTier: users.subscriptionTier, subscriptionStatus: users.subscriptionStatus }).from(users).where(eq4(users.id, session3.userId)).limit(1);
    if (!user) {
    ...
    if (subscriptionTier === "test_premium" && subscriptionStatus === "active") {
      ...
      if (rules.blockedEndpoints.some((endpoint) => path9.startsWith(endpoint))) {
        return res.status(403).json({
...
var razorpayInstance = new Razorpay2({
  key_id: process.env.RAZORPAY_KEY_ID,
  key_secret: process.env.RAZORPAY_KEY_SECRET
});
var PLANS = {
...
async function verifyRazorpayPayment(req, res) {
  try {
    ...
    const expectedSignature = crypto4.createHmac("sha256", process.env.RAZORPAY_KEY_SECRET).update(body.toString()).digest("hex");
    ...
    const [user] = await db.select().from(users).where(eq5(users.id, userId2));
    if (user?.email) {
...
var client2 = new Client({
  ...
  timeout: 0,
  environment: isLiveEnvironment ? Environment.Production : Environment.Sandbox,
  logging: {
...
var ordersController = new OrdersController(client2);
var oAuthAuthorizationController = new OAuthAuthorizationController(client2);
...
async function handlePaymentFailure(customerId, invoiceId, failureReason) {
  try {
    const [user] = await db.select().from(users).where(eq6(users.stripeCustomerId, customerId)).limit(1);
    if (!user) {
...
async function suspendApiAccess(userId2, amountDue, reason) {
  try {
    ...
    }).where(eq6(apiKeys.userId, userId2));
    await db.update(users).set({
...
async function checkOutstandingDebt(userId2) {
  try {
    const [user] = await db.select().from(users).where(eq6(users.id, userId2)).limit(1);
...
var ApiKeyManager = class {
  ...
  static generateApiKey() {
    const randomBytes = crypto5.randomBytes(24).toString("hex");
    const fullKey = `sk_test_${randomBytes}`;
    const keyHash = bcrypt3.hashSync(fullKey, 10);
  ...
  static async validateApiKey(providedKey) {
    ...
    const activeKeys = await db.select().from(apiKeys).innerJoin(users, eq7(apiKeys.userId, users.id)).where(and4(
      eq7(apiKeys.isActive, true)
      // Check if key hasn't expired
    ));
    for (const { api_keys: key, users: user } of activeKeys) {
      if (bcrypt3.compareSync(providedKey, key.keyHash)) {
  ...
  static async checkRateLimit(apiKeyId, rateLimit) {
    ...
    const recentUsage = await db.select().from(apiUsage).where(and4(
      eq7(apiUsage.apiKeyId, apiKeyId)
...
var specialFormatUpload = multer({
  storage: storage2,
  limits: {
...
router2.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const tempDir = path4.join(process.cwd(), "temp");
      const uploadsDir = path4.join(tempDir, "uploads");
      const outputDir = path4.join(tempDir, "compressed");
      await fs4.mkdir(uploadsDir, { recursive: true });
      await fs4.mkdir(outputDir, { recursive: true });
      const fileId = uuidv4();
      const originalExtension = path4.extname(req.file.originalname);
      const inputPath = path4.join(uploadsDir, `${fileId}${originalExtension}`);
      await fs4.writeFile(inputPath, req.file.buffer);
      ...
      const outputPath = path4.join(outputDir, `compressed_${fileId}${outputExtension}`);
      ...
      const originalStats = await fs4.stat(inputPath);
      const compressedStats = await fs4.stat(outputPath);
      const compressionRatio = Math.round((originalStats.size - compressedStats.size) / originalStats.size * 100);
      const compressedBuffer = await fs4.readFile(outputPath);
      await fs4.unlink(inputPath).catch(() => {
      });
      await fs4.unlink(outputPath).catch(() => {
      });
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const file of files) {
        try {
          const fileId = uuidv4();
          const originalExtension = path4.extname(file.originalname);
          const inputPath = path4.join(uploadsDir, `${fileId}${originalExtension}`);
          await fs4.writeFile(inputPath, file.buffer);
...
router2.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const tempDir = path4.join(process.cwd(), "temp");
      const uploadsDir = path4.join(tempDir, "uploads");
      const outputDir = path4.join(tempDir, "converted");
      await fs4.mkdir(uploadsDir, { recursive: true });
      await fs4.mkdir(outputDir, { recursive: true });
      const fileId = uuidv4();
      const originalExtension = path4.extname(req.file.originalname);
      ...
      const outputExtension = outputFormat === "jpeg" ? "jpg" : outputFormat;
      const outputPath = path4.join(outputDir, `converted_${fileId}.${outputExtension}`);
      ...
      const compressedStats = await fs4.stat(outputPath);
      const compressionRatio = Math.round((originalStats.size - compressedStats.size) / originalStats.size * 100);
      const convertedBuffer = await fs4.readFile(outputPath);
      await fs4.unlink(inputPath).catch(() => {
      });
      await fs4.unlink(outputPath).catch(() => {
      });
...
async function convertRawFileAPI(inputPath, outputPath, outputFormat, quality, resize, width, height, maintainAspect) {
  ...
  const tiffExists = await fs4.access(tiffPath).then(() => true).catch(() => false);
  if (!tiffExists) {
  ...
  await fs4.unlink(tiffPath).catch(() => {
  });
  return { success: true };
}
...
async function convertTiffFileAPI(inputPath, outputPath, outputFormat, quality, resize, width, height, maintainAspect) {
  ...
  let sharpInstance = sharp3(inputPath);
  if (resize === "true" && width && height) {
    const resizeOptions = {
...
router4.get("/keys", isAuthenticated2, async (req, res) => {
  try {
    ...
    }).from(apiKeys).where(eq8(apiKeys.userId, userId2)).orderBy(apiKeys.createdAt);
...
router4.post("/keys", isAuthenticated2, async (req, res) => {
  try {
    ...
    const existingKeys = await db.select({ count: apiKeys.id }).from(apiKeys).where(and5(
      eq8(apiKeys.userId, userId2),
...
router4.put("/keys/:keyId", isAuthenticated2, async (req, res) => {
  try {
    ...
    const updateSchema = z.object({
      ...
      rateLimit: z.number().min(100).max(1e4).optional(),
    ...
    const [updatedKey] = await db.update(apiKeys).set(updateData).where(and5(
      eq8(apiKeys.id, keyId),
      eq8(apiKeys.userId, userId2)
    )).returning();
    if (!updatedKey) {
      return res.status(404).json({
...
router4.delete("/keys/:keyId", isAuthenticated2, async (req, res) => {
  try {
    const userId2 = req.user.id;
    const keyId = req.params.keyId;
    const [deactivatedKey] = await db.update(apiKeys).set({ isActive: false }).where(and5(
      eq8(apiKeys.id, keyId),
      eq8(apiKeys.userId, userId2)
    )).returning();
    if (!deactivatedKey) {
...
router4.get("/keys/:keyId/usage", isAuthenticated2, async (req, res) => {
  try {
    const userId2 = req.user.id;
    const keyId = req.params.keyId;
    const [apiKey] = await db.select().from(apiKeys).where(and5(
      eq8(apiKeys.id, keyId),
      eq8(apiKeys.userId, userId2)
    ));
    if (!apiKey) {
...
function generateBrandedFilename(originalFilename, inputFormat, outputFormat, operation = "compress", includeTimestamp = false) {
  const nameWithoutExt = path6.parse(originalFilename).name;
  const formatMap = {
    "jpeg": "jpg",
    "jpg": "jpg",
    "png": "png",
...
`;
  files.forEach((file, index) => {
    const ext = path6.extname(file.name).slice(1).toLowerCase();
...
async function processCompressionJob(jobId, originalPath, originalFilename, settings, userId2, sessionId, userType = "anonymous", pageIdentifier = "free-no-auth") {
  try {
    ...
    const outputPath = path6.join("converted", `${jobId}.${outputExtension}`);
    ...
    const originalStats = await fs6.stat(originalPath);
    const compressionRatio = originalStats.size > 0 ? Math.round((originalStats.size - result.finalSize) / originalStats.size * 100) : 0;
    let thumbnailPath;
    try {
      const previewsDir = path6.join(process.cwd(), "previews");
      await fs6.mkdir(previewsDir, { recursive: true });
      thumbnailPath = path6.join(previewsDir, jobId + "_thumb.jpg");
...
async function ensureDirectories() {
  try {
    await fs6.mkdir("uploads", { recursive: true });
    await fs6.mkdir("compressed", { recursive: true });
  } catch (error) {
...
async function registerRoutes(app2) {
  ...
  app2.use("/webhooks", express.raw({ type: "application/json" }), router3);
  ...
  app2.get("/api/preview/:id", (req, res) => {
    try {
      ...
      const previewPath = path6.join(process.cwd(), "previews", id + "_thumb.jpg");
      fs6.access(previewPath).then(() => {
  ...
  app2.get("/api/download/wordpress-plugin", (req, res, next) => {
    const pluginPath = path6.join(process.cwd(), "micro-jpeg-api-wordpress-plugin.zip");
    ...
    try {
      ...
      const findAndServeFile = async () => {
        try {
          ...
          if (job && job.outputFormat) {
            ...
            for (const dir of directories) {
              const filePath = path6.join(process.cwd(), dir, `${id}.${correctExt}`);
              try {
                await fs6.access(filePath);
                const contentType = getMimeTypeForDownload(correctExt);
                const inputFormat = job.inputFormat || path6.extname(job.originalFilename || "").slice(1) || "jpg";
        ...
        for (const ext of possibleExtensions) {
          const convertedPath = path6.join(process.cwd(), "converted", `${id}.${ext}`);
          try {
            await fs6.access(convertedPath);
            ...
            const stats = await fs6.stat(convertedPath);
            res.setHeader("Content-Type", contentType);
            res.setHeader("Content-Length", stats.size);
            res.setHeader("Content-Disposition", `attachment; filename="${brandedFilename}"`);
            res.setHeader("Cache-Control", "public, max-age=3600");
        ...
        for (const ext of possibleExtensions) {
          const compressedPath = path6.join(process.cwd(), "compressed", `${id}.${ext}`);
          try {
            await fs6.access(compressedPath);
            const contentType = getMimeTypeForDownload(ext);
            const brandedFilename = generateBrandedFilename(
              `download_${id}`,
              ext,
            ...
            const stats = await fs6.stat(compressedPath);
            res.setHeader("Content-Type", contentType);
            res.setHeader("Content-Length", stats.size);
            res.setHeader("Content-Disposition", `attachment; filename="${brandedFilename}"`);
            res.setHeader("Cache-Control", "public, max-age=3600");
    ...
    try {
      ...
      for (const { job, file, outputFormat } of jobs) {
        try {
          ...
          const outputPath = path6.join("compressed", `${job.id}.${fileExtension}`);
          ...
          if (isRawFile || inputFormat === "svg" || inputFormat === "svg" && outputFormat === "tiff") {
            ...
            let sharpOperation = sharp5(file.path);
          ...
          const originalStats = await fs6.stat(file.path);
          const compressedStats = await fs6.stat(outputPath);
          ...
          const resultData = {
            ...
            compressedFileName: path6.basename(outputPath),
            settings: {
              quality: settings.quality,
  ...
  app2.get("/api/download/compressed/:jobId", async (req, res) => {
    try {
      ...
      const baseName = path6.parse(originalName).name;
      ...
      await fs6.access(job.compressedPath);
      res.setHeader("Content-Disposition", `attachment; filename="${downloadName}"`);
  ...
  app2.get("/api/image/:jobId", async (req, res) => {
    try {
      ...
      res.sendFile(path6.resolve(job.compressedPath));
    } catch (error) {
  ...
  app2.get("/api/download-zip/:batchId", async (req, res) => {
    try {
      ...
      for (const filename of batchInfo.files) {
        try {
          let filePath = path6.join(compressedDir, filename);
          try {
            await fs6.access(filePath);
            validFiles.push({
              name: filename,
            ...
            filePath = path6.join("converted", filename);
            await fs6.access(filePath);
            validFiles.push({
              name: filename,
              path: filePath
      ...
      for (const file of validFiles) {
        try {
          await fs6.access(file.path);
          const fileExt = path6.extname(file.name).slice(1).toLowerCase();
          const format = normalizeFormat(fileExt);
          const cleanName = file.name.replace(/^compressed_\d+_/, "");
          const baseNameWithoutExt = path6.parse(cleanName).name;
          const brandedName = generateBrandedFilename(
            baseNameWithoutExt + path6.extname(cleanName),
            format,
            format,
            "compress",
      ...
      for (const resultId of resultIds) {
        try {
          ...
          if (job && job.compressedPath && job.status === "completed") {
            await fs6.access(job.compressedPath);
            const filename = path6.basename(job.compressedPath);
            validFiles.push({
              name: filename,
            ...
            for (const ext of specialFormatExtensions) {
              const potentialPath = path6.join("converted", `${resultId}.${ext}`);
              try {
                await fs6.access(potentialPath);
            ...
            if (filePath) {
              ...
              validFiles.push({
                name: filename,
                path: filePath,
                originalName: `converted_image.${path6.extname(filePath).substring(1)}`
              });
            }
          }
        } catch (err) {
  ...
  app2.get("/api/jobs/:id", isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      if (job.originalPath) {
        try {
          const imageBuffer = await fs6.readFile(job.originalPath);
      ...
      if (job.status === "completed" && job.compressedPath) {
        try {
          const compressedBuffer = await fs6.readFile(job.compressedPath);
  ...
  app2.get("/api/preview/original/:id", async (req, res) => {
    try {
      ...
      try {
        await fs6.access(job.originalPath);
    ...
    try {
      ...
      if (!job || !job.compressedPath) {
        ...
        for (const ext2 of specialFormatExtensions) {
          const potentialPath = path6.join("converted", `${req.params.id}.${ext2}`);
          try {
            await fs6.access(potentialPath);
            filePath = potentialPath;
            break;
        ...
        const stats = await fs6.stat(filePath);
        const ext = path6.extname(filePath).substring(1).toLowerCase();
        let contentType2;
        switch (ext) {
          case "png":
      ...
      try {
        const stats = await fs6.stat(job.compressedPath);
  ...
  app2.delete("/api/jobs/:id", isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      try {
        try {
          await fs6.access(job.originalPath);
          await fs6.unlink(job.originalPath);
        ...
        if (job.compressedPath) {
          try {
            await fs6.access(job.compressedPath);
            await fs6.unlink(job.compressedPath);
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          const fileBuffer = await fs6.readFile(file.path);
          console.log(`File buffer size: ${fileBuffer.length}`);
          if (fileBuffer.length === 0) {
            throw new Error("Empty file buffer");
          }
          const compressedBuffer = await sharp5(fileBuffer).jpeg({ quality: GUEST_QUALITY, progressive: true }).toBuffer();
  ...
  app2.post("/api/reset-raw-usage", async (req, res) => {
    try {
      ...
      const result = await db.update(userUsage).set({
        ...
        rawHourly: 0,
        updatedAt: /* @__PURE__ */ new Date()
      }).where(eq9(userUsage.userId, "anonymous"));
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          const convertedStats = await fs6.stat(outputPath);
          const job = {
            id: jobId,
            originalFilename: file.originalname,
            originalSize: originalStats.size,
          ...
          let downloadUrl;
          try {
            const thumbnailPath = await generateThumbnailFromRaw(outputPath, path6.dirname(outputPath), jobId);
            if (thumbnailPath) {
...
async function compressImage(jobId, file, options) {
  try {
    ...
    let sharpInstance = sharp5(file.path, {
    ...
    if (options.resizeOption !== "none" && metadata.width && metadata.height) {
      ...
      sharpInstance = sharpInstance.resize(targetWidth, targetHeight, {
        kernel: options.fastMode ? sharp5.kernel.nearest : sharp5.kernel.lanczos3,
    ...
    const outputPath = path6.join("compressed", `${jobId}.${fileExtension}`);
...
async function generateThumbnailFromRaw(inputPath, outputDir, jobId) {
  try {
    const previewsDir = path6.join(process.cwd(), "previews");
    await fs6.mkdir(previewsDir, { recursive: true });
    const thumbnailPath = path6.join(previewsDir, jobId + "_thumb.jpg");
...
async function processSpecialFormatConversion(inputPath, outputPath, inputFormat, outputFormat, options = {
  ...
  try {
    if (inputFormat === "raw") {
      ...
      const inputExists = await fs6.access(inputPath).then(() => true).catch(() => false);
      ...
      const tiffExists = await fs6.access(tempTiffPath).then(() => true).catch(() => false);
      ...
      if (outputFormat === "tiff") {
        const outputFilename = path6.basename(outputPath, path6.extname(outputPath));
        thumbnailPromise = generateThumbnailFromRaw(tempTiffPath, path6.dirname(outputPath), outputFilename);
      }
      await execAsync3(convertCommand);
      const outputExists = await fs6.access(outputPath).then(() => true).catch(() => false);
      console.log(`Output file created: ${outputExists}`);
      let previewPath2;
      if (outputFormat === "tiff") {
        try {
          const previewsDir = path6.dirname(outputPath).replace("/converted", "/previews");
          await fs6.mkdir(previewsDir, { recursive: true });
          const outputFilename = path6.basename(outputPath, path6.extname(outputPath));
          previewPath2 = path6.join(previewsDir, outputFilename + ".jpg");
      ...
      sharpInstance = sharp5(inputPath, {
        density: 300
    ...
    if (outputFormat === "tiff") {
      try {
        ...
        await fs6.mkdir(previewsDir, { recursive: true });
        const outputFilename = path6.basename(outputPath, path6.extname(outputPath));
        previewPath = path6.join(previewsDir, outputFilename + ".jpg");
        await sharp5(outputPath).resize(512, 512, { fit: "inside", withoutEnlargement: true }).jpeg({ quality: 70, progressive: true }).toFile(previewPath);
...
function registerResetEndpoints(app2) {
    ...
    try {
      ...
      try {
        await db.delete(anonymousSessionScopes).where(
          and6(
            eq9(anonymousSessionScopes.sessionId, sessionId),
            eq9(anonymousSessionScopes.scope, pageIdentifier)
          )
        );
      ...
      try {
        const userUsageDeleted = await db.delete(userUsage).where(
          and6(
            eq9(userUsage.sessionId, sessionId),
            eq9(userUsage.userId, "anonymous")
          )
        );
      ...
      try {
        await db.delete(operationLog).where(eq9(operationLog.sessionId, sessionId));
  ...
  app2.post("/api/dev-nuclear-reset", async (req, res) => {
    try {
      ...
      let clearedItems = 0;
      try {
        const result = await db.delete(userUsage).where(eq9(userUsage.userId, "anonymous"));
      ...
      try {
        await db.delete(operationLog).where(eq9(operationLog.userId, null));
  ...
  app2.post("/api/admin/counters/reset", ensureSuperuser, async (req, res) => {
    try {
      ...
      let whereCondition;
      if (userId2) {
        whereCondition = eq9(userUsage.userId, userId2);
      } else {
        whereCondition = eq9(userUsage.sessionId, sessionId);
      }
...
var vite_config_default = defineConfig({
  plugins: [react()],
  root: path7.resolve(__dirname2, "client"),
  resolve: {
    alias: {
      "@": path7.resolve(__dirname2, "client/src"),
      "@assets": path7.resolve(__dirname2, "client/src/assets"),
      "@shared": path7.resolve(__dirname2, "shared")
    }
  },
  build: {
    outDir: path7.resolve(__dirname2, "dist"),
    emptyOutDir: true
  },
  server: {
    port: 3e3,
...
function serveStatic(app2) {
  const distPath = path8.resolve(__dirname, "../dist/public");
...
init_db();
init_schema();
import { eq as eq10, and as and7, lt } from "drizzle-orm";
var TestPremiumExpiryManager = class {
  ...
  static async checkExpiredSubscriptions() {
    try {
      ...
      const expiredUsers = await db.select().from(users).where(
        and7(
          eq10(users.subscriptionTier, "test_premium"),
          eq10(users.subscriptionStatus, "active"),
  ...
  static async getTimeUntilExpiry(userId2) {
    try {
      const [user] = await db.select().from(users).where(eq10(users.id, userId2)).limit(1);
...
app.use(express3.urlencoded({ extended: false, limit: "200mb" }));
...
(async () => {
  try {
    ...
    console.log("\u{1F4DD} Note: Set REDIS_URL environment variable for queue functionality");
  }
  ...
  console.log("\u{1F50D} Registered routes:");
